...
public static class DownloadPipeline
{
    ...
    /// </summary>
    public static async Task RunAsync(DownloadRequest req, PipelineSink sink = default, ChannelWorkflowContext? workflow = null, CancellationToken ct = default)
    {
    ...
    internal static PipelineSink ComposeSink(PipelineSink sink, ChannelWorkflowContext? workflow)
    {
        if (workflow is null)
        {
            return sink;
