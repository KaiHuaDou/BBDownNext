...
| **AOT 原生发布**    | 未提供；依赖运行时反射                                                    | 代码已改造为 **AOT 安全**（`System.Text.Json` 源生成器替代反射）；AOT 在 `BBDown/Directory.Build.props` 中默认开启（`<PublishAot>true</PublishAot>`），直接 `dotnet publish BBDown -r <RID> -c Release` 即产出单文件原生二进制（CI 命令见 `.github/workflows/ci.yml`） |
...
| **测试覆盖**        | 较少                                                                      | **650+ 单元测试**（Core + BBDown.Tests，含 gRPC 打包往返、cheese 过滤、serve 安全、断点续传清单、文件名截断等）                                                                                                                                                        |
...
- **文件名截断 / 日期格式**：`BBDown.Core/Util/FileNameUtil.cs`（`MaxBytes = 200`）、`BBDown/SavePath.cs`（`Format` 方法的 `<publishDate:格式>` / `<videoDate:格式>` 自定义日期解析）。
