...
function describeUrl(text: string): string {
  ...
  if (spaceHost && /\/lists\//i.test(text)) {
    return /type=series/i.test(text) ? '系列地址' : '合集地址'
  }

  if (/\/channel\/collectiondetail/i.test(text)) {
    return '合集地址'
  }

  if (/\/channel\/seriesdetail/i.test(text)) {
    return '系列地址'
  }

  if (/medialist\/(?:play|detail)\/ml[0-9]+/i.test(text)) {
    return '合集地址'
  }
