...
- 外部后处理（`--post-process`）配置由进程级全局改为按任务透传：路径随 `DownloadRequest` 流入，GUI 并发任务各自生效、互不覆盖；未配置时整体不启用。

## [v2.0.0]

### 新增

- 分片级重试：分片瞬态故障由 downloader 库退避重试（上限 5 次）并从断点续下，避免整 P 退避重下。
- FLV 分段并行下载：片段间并行（上限 4），且与片段内 downloader 并行连接合计不超过 32（原片段间串行）。
...
- 图形界面扫码登录：弹出窗口支持 WEB / TV / APP 三通道扫码，凭据写入 `BBDown.data`，登录态显示昵称。
- 图形界面接通直播录制与专栏导出链路：任务执行按目标分流到直播 / 专栏（此前界面可识别但无法执行），运行中的直播任务提供「停止录制并合并」按钮（对应 CLI 的 Ctrl+Break 语义）。
- 图形界面队列持久化：关闭时保存未完成（等待 / 运行）任务到 `BBDown.GUI.queue.json`，启动时恢复为等待。
- 新增 `BBDown.GUI.Tests` 纯逻辑单元测试（目标识别 / 参数映射 / 状态转换器 / 任务队列，37 例）。
...
- 图形界面队列状态栏拆分为等待 / 运行 / 成功 / 失败 / 已取消计数；日志区改为 ListBox 虚拟化渲染并新增「导出日志」按钮。
- 图形界面登录态探测覆盖 WEB / TV / APP 三类凭据（TV / APP 登录后不再误显示「未登录」）。
- 登录编排重构：抽取 `WebCredentialAsync` / `TvCredentialAsync` / `AppCredentialAsync` 与 `GenerateQrPng`，成功返回凭据数据由调用方处置（GUI 落盘复用，CLI 行为不变）。
- 外部后处理触发改为全轨调起：`--post-process` 配置后对所有 DASH 轨发起后处理，是否加密由处理方自行判断（退出码 0 且无产物视为无需处理）；主程序移除 playurl 加密标记解析（`widevine_pssh` / `bilidrm_uri`）与轨道 `IsEncrypted` 字段。
...
- 日志输出目标可注入：新增 `Logger.Output` 委托，null 时写控制台（含颜色与 `BeforeWrite` 钩子），GUI 等无控制台宿主可替换为自定义输出（如窗口日志区），`BeforeWrite` 仅对默认控制台路径生效。
...
- 外部后处理插件架构：新增 `--post-process <exe>`，下载完成后带加密标记的轨道文件交由外部进程处理，成功产物替换原文件参与混流；进程不可用或处理失败时静默保留原文件（加密流照常输出）。主程序不再内置解密能力，原密钥选项与图形界面输入项移除，密钥与加密信息均由外部进程自行获取管理。
