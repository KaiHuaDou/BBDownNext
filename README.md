...
- **serve 任务管线**：REST API 规范化（`/api/v1/tasks`，202 受理 / 200 重复 / 400 非法 / 429 限流的完整语义）、任务队列 + 并发闸门（`--max-concurrent` 真正限并发、排队任务可取消）、WebSocket 事件流（`--no-interactive` 关闭，推送消息 / 进度快照 / 选项请求，选项可远程应答）。
...
- 扩展与集成
    ...
    - **任务事件流** · WebSocket `/hubs/tasks`（`--no-interactive` 关闭），任务消息 / 进度快照 / 选项请求实时推送，`submitChoice` 帧远程应答选项
...
| serve API       | `/add-task` 等散点端点          | `/api/v1/tasks` 规范 REST（202 受理 / 200 重复 / 400 非法 / 429 限流）+ WebSocket 事件流（`--no-interactive` 关闭：消息 / 进度快照 / 选项远程应答）          |
...
| `--no-interactive` | `-ni` | 关闭任务事件流（默认开启，经 WebSocket `/hubs/tasks` 推送消息 / 进度 / 选项请求），选项可远程应答；指定本项后服务端不推送事件流、无额外开销，交互选项回落非交互                                          |
