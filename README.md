...
BBDown 的下载过程是一条清晰的流水线：从链接解析出可用音视频流，按优先级选出目标清晰度，再下载、混流并输出为本地文件。

![BBDown 下载流程](docs/assets/architecture.svg)
...
- 解析模式类参数以 `--tv-api` / `--app-api` / `--intl-api` 等形式表示，去掉了旧版的 `--use-` 前缀
- 仅下载某一类内容的参数提供单字母简写：**`-v` 视频 / `-a` 音频 / `-s` 字幕 / `-c` 封面 / `-d` 弹幕**
- 配置文件参数统一为 `--config`（旧版 `--config-file` 已移除）

命令行参数会先经过 `System.CommandLine` 解析，组装成强类型配置对象 `MyOption`，再交给任务编排入口 `RunApp` 执行。

![命令行参数解析流程](docs/assets/cli-workflow.svg)
...
| `serve`   | 以服务器模式运行（默认监听 `http://0.0.0.0:23333`） |
