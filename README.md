...
| serve 安全      | 基础令牌                        | SSRF 防护、CORS 默认仅回环来源放行、host 与工作目录固定、请求凭据门（Cookie 仅发往官方域或配置 host）、取消令牌贯通、ResourceId 标识天然去重、全局/任务提交限流、认证失败滑动窗口、Origin 校验、错误消息脱敏 |
...
| `--interactive`    |      | 开启任务事件流（WebSocket `/hubs/tasks` 推送消息 / 进度 / 选项请求），选项可远程应答；                                          |
...
显式传入 `--serve-token` 后强制所有访问鉴权（未传则默认免令牌并告警）；令牌为 `X-BBDown-Token` 头或 `?token=` 查询参数；回调地址有 SSRF 防护（拒绝内网 / 回环，连接前二次校验）；CORS 默认仅回环来源放行；host 与工作目录由服务端启动参数固定，请求体无法覆盖；携带 Cookie 的请求经凭据门只发往官方域或配置的 host；`Ctrl+C` 关停可中断排队中任务的解析请求。
